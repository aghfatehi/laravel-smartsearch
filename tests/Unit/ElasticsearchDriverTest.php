...
class ElasticsearchDriverTest extends TestCase
{
    ...
    public function test_search_returns_matching_models_from_es_ids(): void
    {
        $response = $this->mockSearchResponse(['1']);
        $client = \Mockery::mock(ClientInterface::class);
        $client->shouldReceive('search')->once()->andReturn($response);

        $driver = new ElasticsearchDriver(['localhost:9200'], $client);
    ...
    public function test_delete_ignores_404(): void
    {
        ...
        $client->shouldReceive('delete')->once()->andThrow(
            new ClientResponseException('not found', 404)
        );
    ...
    private function mockSearchResponse(array $ids): Elasticsearch
    {
        $hits = [];
        foreach ($ids as $id) {
        ...
        $response->shouldReceive('offsetExists')->with('hits')->andReturn(true);
        $response->shouldReceive('offsetGet')->with('hits')->andReturn(['hits' => $hits]);
